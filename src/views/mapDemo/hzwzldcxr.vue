...
import type { MouseEventListenOptions, MouseEventPickPayload, LabelSnapshot } from '../../CesiumX'
...
const tableData = ref<LabelSnapshot[]>([])
...
function refreshTable(): void {
  ...
  const L = window.XGX?.Label
  if (!v || v.isDestroyed() || !L) {
    tableData.value = []
    return
  }
  tableData.value = L.getAllLabels(v)
  void nextTick(() => updateTableScrollY())
}

function fillFormFromSnapshot(s: LabelSnapshot): void {
  form.id = s.id
  form.longitude = s.longitude
  form.latitude = s.latitude
  form.height = s.height
  form.text = s.text
  form.font = s.font ?? '14px sans-serif'
  ...
  const fillCss =
    s.fillColorCss ??
    (typeof td.fillColor === 'string' && td.fillColor.trim()
      ? String(td.fillColor)
      : typeof td.fontColor === 'string' && td.fontColor.trim()
        ? String(td.fontColor)
        : undefined)
  ...
  form.outline = (s.outlineWidth ?? 0) > 0
  const outCss =
    s.outlineColorCss ??
  ...
  form.showBackground = s.showBackground ?? false
  const bgCss =
    s.backgroundColorCss ??
    (typeof td.backgroundColor === 'string' && td.backgroundColor.trim()
      ? String(td.backgroundColor)
      : undefined)
  form.backgroundColor = parseCssColorForForm(bgCss, '#1a1a2e').hex
  form.show = s.show
}
...
function onColorPick(field: 'color' | 'outlineColor' | 'backgroundColor', ev: Event): void {
  const el = ev.target as HTMLInputElement
  if (field === 'color') {
    form.color = normalizeHex(el.value, DEFAULT_FILL_COLOR)
  } else if (field === 'outlineColor') {
    form.outlineColor = normalizeHex(el.value, DEFAULT_OUTLINE_COLOR)
  } else {
    form.backgroundColor = normalizeHex(el.value, '#1a1a2e')
  }
}
...
function applyUpdateToSelected(): void {
  ...
  const L = window.XGX?.Label
  const v = mapStore.getViewer()
  if (!id || !L || !v || v.isDestroyed()) return
  const ok = L.updateLabel(id, {
...
function tableRowClassName(record: LabelSnapshot): string {
  return record.id === selectedId.value ? 'hzd-point-row--active' : ''
}

function customTableRow(record: LabelSnapshot) {
  return {
...
<template>
  <div class="map-tool-float map-tool-float--hzd-point map-tool-float--hzd-label">
      ...
      <div class="hzd-dialog-body">
        ...
        <div class="hzd-shell">
            ...
            <div class="hzd-pane-scroll hzd-scroll-skin">
              <div class="hzd-form-fields">
                <div class="hzd-field-row">
                  <span class="hzd-field-label">标签 ID</span>
                ...
                <div class="hzd-field-row">
                  <span class="hzd-field-label">文本内容</span>
                  <div class="hzd-field-control">
                    <a-input v-model:value="form.text" class="hzd-control-fill" size="small" allow-clear placeholder="显示在地图上的文字" />
                  </div>
                </div>

                <div class="hzd-field-row">
                  <span class="hzd-field-label">字体</span>
                  <div class="hzd-field-control">
                    <a-input v-model:value="form.font" class="hzd-control-fill" size="small" placeholder="如 14px sans-serif" />
                  </div>
                </div>
                ...
                <div class="hzd-field-row">
                  ...
                  <div class="hzd-field-control">
                    <a-input-number v-model:value="form.scale" class="hzd-control-fill" size="small" :min="0.2" :max="4" :step="0.1" />
                  </div>
                </div>
                ...
                <div class="hzd-field-row">
                  <span class="hzd-field-label">文字背景框</span>
                  <div class="hzd-field-control">
                    <a-switch v-model:checked="form.showBackground" size="small" />
                  </div>
                </div>

                <div class="hzd-field-row">
                  <span class="hzd-field-label">背景颜色</span>
                  <div class="hzd-field-control">
                    <label class="hzd-color-native">
                      <span class="hzd-swatch" :style="{ backgroundColor: form.backgroundColor }" aria-hidden="true" />
                      <input
                        type="color"
                        class="hzd-color-hit"
          ...
          <section class="hzd-pane hzd-pane--table">
            <div class="hzd-pane-title">标签列表</div>
