...
import type { EllipsoidSnapshot, MouseEventListenOptions, MouseEventPickPayload } from '../../CesiumX'
...
const DEFAULT_FILL_COLOR = '#6495ed'
...
const tableData = ref<EllipsoidSnapshot[]>([])
...
function refreshTable(): void {
  ...
  const E = window.XGX?.Ellipsoid
  if (!v || v.isDestroyed() || !E) {
    tableData.value = []
    return
  }
  tableData.value = E.getAllEllipsoids(v)
  void nextTick(() => updateTableScrollY())
}

function fillFormFromSnapshot(s: EllipsoidSnapshot): void {
  form.id = s.id
  form.longitude = s.longitude
  form.latitude = s.latitude
  form.height = s.height
  form.radiusX = s.radii.x > 0 ? s.radii.x : form.radiusX
  form.radiusY = s.radii.y > 0 ? s.radii.y : form.radiusY
  form.radiusZ = s.radii.z > 0 ? s.radii.z : form.radiusZ
  ...
  const owSnap = typeof s.outlineWidth === 'number' && Number.isFinite(s.outlineWidth) ? s.outlineWidth : undefined
  const owTd = typeof td.outlineWidth === 'number' && Number.isFinite(td.outlineWidth) ? td.outlineWidth : undefined
  const rawOutline =
    typeof s.outline === 'boolean' ? s.outline : typeof td.outline === 'boolean' ? td.outline : undefined
  const effectiveW = owSnap ?? owTd
  form.outline = rawOutline !== undefined ? rawOutline : (effectiveW ?? 0) > 0
  const ow = form.outline ? Math.max(1, effectiveW ?? 1) : 0
  const outCss =
    s.outlineColorCss ??
...
function onRowClick(record: EllipsoidSnapshot): void {
  ...
  const snap = window.XGX?.Ellipsoid?.getEllipsoid(record.id)
  if (snap) fillFormFromSnapshot(snap)
}

function onDeleteRow(id: string, e: Event): void {
  e.stopPropagation()
  window.XGX?.Ellipsoid?.remove(id)
...
function applyUpdateToSelected(): void {
  ...
  if (ok) {
    ...
    refreshTable()
    const snap = E.getEllipsoid(id)
    if (snap) fillFormFromSnapshot(snap)
  } else {
    message.error('保存失败，请确认该椭球仍存在')
  }
}

function addEllipsoidFromForm(): void {
  ...
  if (!entity) {
    message.error('添加失败：id 可能重复或参数无效，请检查后重试')
    return
  }
...
function formatRadiiCell(s: EllipsoidSnapshot): string {
  const { x, y, z } = s.radii
  return `${Math.round(x)}×${Math.round(y)}×${Math.round(z)}`
}

const columns: TableColumnType<EllipsoidSnapshot>[] = [
  ...
  },
  {
    ...
    width: 132,
    align: 'center',
    customRender: ({ record }) => formatRadiiCell(record as EllipsoidSnapshot),
  },
  { title: '操作', key: 'action', width: 56, align: 'center', fixed: 'right' },
]

function tableRowClassName(record: EllipsoidSnapshot): string {
  return record.id === selectedId.value ? 'hzd-ellipsoid-row--active' : ''
}

function customTableRow(record: EllipsoidSnapshot) {
  return {
...
<template>
  <div class="map-tool-float map-tool-float--hzd-ellipsoid">
      ...
      <div class="hzd-dialog-body">
        ...
        <div class="hzd-shell">
            ...
            <div class="hzd-pane-scroll hzd-scroll-skin">
              <div class="hzd-form-fields">
                <div class="hzd-field-row">
                  <span class="hzd-field-label">椭球 ID</span>
                ...
                <div class="hzd-field-row">
                  <span class="hzd-field-label">半径 X（m）</span>
                  <div class="hzd-field-control">
                    <a-input-number
                      v-model:value="form.radiusX"
                      class="hzd-control-fill"
                      size="small"
                      :min="1"
                      :max="2000000"
                ...
                <div class="hzd-field-row">
                  <span class="hzd-field-label">半径 Y（m）</span>
                  <div class="hzd-field-control">
                    <a-input-number
                      v-model:value="form.radiusY"
                      class="hzd-control-fill"
                      size="small"
                      :min="1"
                      :max="2000000"
                ...
                <div class="hzd-field-row">
                  <span class="hzd-field-label">半径 Z（m）</span>
                  <div class="hzd-field-control">
                    <a-input-number
                      v-model:value="form.radiusZ"
                      class="hzd-control-fill"
                      size="small"
                      :min="1"
                      :max="2000000"
          ...
          <section class="hzd-pane hzd-pane--table">
            <div class="hzd-pane-title">椭球列表</div>
...
.map-tool-float--hzd-ellipsoid :deep(.x-dialog-panel) {
  display: flex;
  flex-direction: column;
  overflow: hidden;
}

.map-tool-float--hzd-ellipsoid :deep(.x-dialog-inner) {
  flex: 1;
  min-height: 0;
  display: flex;
  flex-direction: column;
