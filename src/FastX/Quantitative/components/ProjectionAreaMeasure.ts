...
import { computeProjectionArea } from '../measureMath'
import { createMeasureAreaLabelText, MeasureBase } from './MeasureBase'

/**
 * 投影面积测量。
 * 多边形顶点在椭球面上的投影面积（二维平面面积算法），面贴地显示。
 */
export class ProjectionAreaMeasure extends MeasureBase {
  /**
   * 根据多边形顶点绘制贴地面并计算投影面积。
  ...
  private refreshGeometry(positions: LngLatHeightTuple[], labelAt?: LngLatHeightTuple): void {
    if (positions.length >= 3) {
      this.setDynamicPolygon(positions, true)
      const area = computeProjectionArea(positions)
      this.updateMeasureLabel(
        labelAt ?? positions[positions.length - 1]!,
        createMeasureAreaLabelText('投影面积', area),
      )
      return
    }
