...
/**
 * CesiumX — 图层（Layer）+ 文档「基础工具（Tools）类」能力封装。
 *
...
export class Layer {
  ...
  private homeState: { center: LayerCenter; orientation: LayerCameraOrientation } | null = null
   ...
   * ```
   */
  async initMap(containerId: string, config: LayerInitConfig): Promise<Cesium.Viewer> {
    this.destroy()
    ...
    this.homeState = { center: { ...config.center }, orientation: { ...orientation } }
    ...
    viewer.scene.globe.depthTestAgainstTerrain = config.depthTestAgainstTerrain ?? false

    this.viewer = viewer

    viewer.scene.screenSpaceCameraController.zoomFactor = config.wheelZoomFactor ?? 2.25

    await this.applyInitialCamera(viewer, config.center, orientation, config.initialCamera)

    if (config.showGridAtStartup) {
      this.addGridImageryLayer(config.gridAtStartupOptions ?? {})
    }
  ...
  async setTerrainWithCesiumTerrainProvider(
    ...
    viewer.terrainProvider = await Cesium.CesiumTerrainProvider.fromUrl(url, options)
  }
   ...
   * ```
   */
  addImageryFromWmts(wmtsOptions: WmtsImageryAddInput, insert?: ImageryLayerInsertOptions): Cesium.ImageryLayer {
    this.assertViewer()
   ...
   * ```
   */
  addImageryFromWms(wmsOptions: WmsImageryAddInput, insert?: ImageryLayerInsertOptions): Cesium.ImageryLayer {
    this.assertViewer()
  ...
  async addImageryFromTms(
    ...
    this.assertViewer()
    const provider = await Cesium.TileMapServiceImageryProvider.fromUrl(url, options)
    return this.addImageryProvider(provider, insert?.insertIndex)
  }
   ...
   * @example
   * ```ts
   * layer.addGridImageryLayer({ cells: 8, color: Cesium.Color.WHITE })
   * ```
   */
  addGridImageryLayer(
    ...
    this.assertViewer()
    const provider = new Cesium.GridImageryProvider(gridOptions ?? {})
   ...
   * ```
   */
  async loadKmlDataSource(data: KmlLoadInput, options?: KmlLoadOptions): Promise<Cesium.KmlDataSource> {
    const viewer = this.assertViewer()
   ...
   * ```
   */
  async loadCzmlDataSource(czml: CzmlLoadInput, options?: CzmlLoadOptions): Promise<Cesium.CzmlDataSource> {
    const viewer = this.assertViewer()
   ...
   * ```
   */
  removeImageryLayer(layer: Cesium.ImageryLayer, destroyImageryProvider?: boolean): void {
    const viewer = this.assertViewer()
   ...
   * ```
   */
  removeImageryLayerByIndex(index: number, destroyImageryProvider?: boolean): void {
    const viewer = this.assertViewer()
    const layer = viewer.imageryLayers.get(index)
    if (layer) viewer.imageryLayers.remove(layer, destroyImageryProvider ?? false)
    if (layer === this.gridOverlayLayer) this.gridOverlayLayer = null
  }
  ...
  setImageryLayerTextureFilters(
    target: number | Cesium.ImageryLayer,
    filters: { minificationFilter?: Cesium.TextureMinificationFilter; magnificationFilter?: Cesium.TextureMagnificationFilter },
  ): void {
    const layer = this.resolveImageryLayer(target)
    if (filters.minificationFilter !== undefined) layer.minificationFilter = filters.minificationFilter
    if (filters.magnificationFilter !== undefined) layer.magnificationFilter = filters.magnificationFilter
  }
   ...
   * ```
   */
  setImageryLayerShow(target: number | Cesium.ImageryLayer, show: boolean): void {
  ...
  resetSharedMapDemoUiState(): void {
    ...
    if (v.scene.mode !== Cesium.SceneMode.SCENE3D && v.scene.mode !== Cesium.SceneMode.MORPHING) {
      v.scene.morphTo3D(0)
    }
  }
   ...
   * ```
   */
  setOverviewMapVisible(visible: boolean, options?: LayerOverviewMapOptions): void {
    ...
    ov.scene.globe.depthTestAgainstTerrain = main.scene.globe.depthTestAgainstTerrain
    ov.scene.screenSpaceCameraController.enableInputs = false

    const template = options?.imageryUrlTemplate ?? this.tryGetUrlTemplateFromMainImagery(main)
    if (!template) {
      ov.destroy()
      container.remove()
      this.overviewContainer = null
      throw new Error('[CesiumX.Layer] 鹰眼需要 imageryUrlTemplate：请在 setOverviewMapVisible 的 options 中传入，或主视图首层为 UrlTemplateImageryProvider')
    }
    ...
    const syncMainToOverview = () => {
      if (!this.viewer || this.viewer.isDestroyed() || !this.overviewViewer || this.overviewViewer.isDestroyed()) {
        return
      }
      const m = this.viewer
    ...
    const removeSync = main.scene.postRender.addEventListener(syncMainToOverview)
   ...
   * ```
   */
  resetCameraToHome(options?: { useAnimation?: boolean; duration?: number }): void | Promise<void> {
    const viewer = this.assertViewer()
    if (!this.homeState) {
  ...
  setMapCenter(
    center: LayerCenter,
    options?: { useAnimation?: boolean; duration?: number; orientation?: LayerCameraOrientation },
  ): void | Promise<void> {
    if (options?.useAnimation) {
      return this.applyFlyToCenter(center, options.orientation, options.duration ?? 2)
    }
   ...
   * ```
   */
  getCameraCenterLngLatHeight(): { longitude: number; latitude: number; height: number } {
    const c = this.assertViewer().camera.positionCartographic
    return {
    ...
    if (visible) {
      ...
      const onPost = () => {
        ...
        if (frustum?.getPixelDimensions) {
          const pixelRatio =
            (typeof window !== 'undefined' ? window.devicePixelRatio : 1) * viewer.resolutionScale
      ...
      this.scaleBarPostRemove = viewer.scene.postRender.addEventListener(onPost)
    } else {
  ...
  setTileLevelOverlayVisible(visible: boolean): void {
    ...
    if (visible) {
      ...
      const onPost = () => {
        ...
        const level = Math.max(0, Math.min(18, Math.round(Math.log2(40075017 / Math.max(h, 1)))))
        this.tileLevelEl.textContent = `层级 ≈ ${level}`
      }
      this.tileLevelPostRemove = viewer.scene.postRender.addEventListener(onPost)
    } else {
  ...
  private handleMainCanvasPointerForOverview(e: MouseEvent): void {
    ...
    let cartesian = main.scene.globe.pick(ray, main.scene) as Cesium.Cartesian3 | undefined
    if (!cartesian) {
      cartesian = main.camera.pickEllipsoid(pos, main.scene.globe.ellipsoid) ?? undefined
    }
  ...
  private applyPerformance(viewer: Cesium.Viewer, perf?: LayerPerformanceInitConfig): void {
    if (!perf) return
    if (perf.requestRenderMode !== undefined) viewer.scene.requestRenderMode = perf.requestRenderMode
    ...
    if (perf.useBrowserRecommendedResolution !== undefined) {
      viewer.useBrowserRecommendedResolution = perf.useBrowserRecommendedResolution
    }
    if (perf.showFps !== undefined) viewer.scene.debugShowFramesPerSecond = perf.showFps
  ...
  private applySetViewToCenter(center: LayerCenter, orientation?: LayerCameraOrientation): void {
    const viewer = this.assertViewer()
  ...
  private applyFlyToCenter(
    ...
    duration = 2,
    overrides?: Omit<LayerCameraFlyToOptions, 'destination' | 'orientation' | 'duration'>,
  ): Promise<void> {
    const viewer = this.assertViewer()
    const destination = Cesium.Cartesian3.fromDegrees(center.longitude, center.latitude, center.height)
    const ori = this.toHeadingPitchRoll(orientation)
    return this.flyToPromise(viewer, { destination, orientation: ori, duration, ...overrides })
  }
  ...
  private applyMainLookAtFromOverview(canvasPosition: Cesium.Cartesian2): void {
    ...
    if (!ray) return
    const hit = ov.scene.globe.pick(ray, ov.scene) as Cesium.Cartesian3 | undefined
    if (!hit) return
    const cart = Cesium.Cartographic.fromCartesian(hit)
    const mainPos = main.camera.positionCartographic
    main.camera.setView({
      destination: Cesium.Cartesian3.fromRadians(cart.longitude, cart.latitude, mainPos.height),
      orientation: {
  ...
  private flyToPromise(viewer: Cesium.Viewer, options: LayerCameraFlyToOptions): Promise<void> {
  ...
  private toHeadingPitchRoll(orientation?: LayerCameraOrientation): Cesium.HeadingPitchRoll {
    const o = orientation ?? {}
  ...
  private tryGetUrlTemplateFromMainImagery(main: Cesium.Viewer): string | undefined {
  ...
  private pickCenter(): { lon: number; lat: number } {
    ...
    const pos = new Cesium.Cartesian2(canvas.clientWidth / 2, canvas.clientHeight / 2)
  ...
  private assertViewer(): Cesium.Viewer {
    if (!this.viewer || this.viewer.isDestroyed()) {
      throw new Error('[CesiumX.Layer] 请先调用 initMap 完成地图初始化')
    }
    return this.viewer
  }

  private addImageryProvider(provider: Cesium.ImageryProvider, insertIndex?: number): Cesium.ImageryLayer {
    const viewer = this.assertViewer()
  ...
  private resolveImageryLayer(target: number | Cesium.ImageryLayer): Cesium.ImageryLayer {
    const viewer = this.assertViewer()
    if (typeof target === 'number') {
      const layer = viewer.imageryLayers.get(target)
      if (!layer) throw new Error(`[CesiumX.Layer] 未找到索引为 ${target} 的影像层`)
      return layer
    }
    return target
  }
